=== Linkable Title Html and Php Widget ===
Contributors: peplamb
Donate link: http://peplamb.com/donate/
Tags: peplamb, widget, clickable, linkable, linked title, text, php, linked title text, linked, Text Widget, Php Widget, Clickable Title Text Widget, Linkable Title Text Widget, Clickable Title php Widget, Linkable Title php Widget
Requires at least: 2.8.0
Tested up to: 3.5.1
Stable tag: trunk

A widget where you may have Text, HTML, Javascript, Flash and/or Php as content with linkable/clickable widget title. 

== Description ==

Using this wordpress plugin you may have Text, HTML, Javascript, Flash and/or Php as content in this widget with linkable/clickable widget titles, so this is a plus compared to the default wordpress' text widget. 

= NEW =
* Added Plugin Options Page (Settings -&gt; Linkable Title Html and Php Widget ).
* Added "Open the Title link/url in a new window" support.
* Added "Title Text Color" support as Katie, Slips, Jim, Gary and John Herfindal request.
* Added support to remove Text Content Div Tag if needed.


= More plugins by PepLamb: =
* <a target="_blank" href="http://peplamb.com/google-analytics-visits/">Google Analytics Visits</a>
* <a target="_blank" href="http://peplamb.com/custom-field-cookie/">Custom Field Cookie</a>

== Installation ==

Installing should be a piece of cake and take fewer than five minutes.

1. Download it
2. Extract all files from the zip archive
3. Copy the linkable-title-html-and-php-widget folder to wp-content/plugins/
4. Activate the plugin through the "Plugins" menu in wordpress
5. Add the widget to the sidebar and configure the options as desired

== Frequently Asked Questions ==

1. Nothing as of now

== Screenshots ==

Please visit <a href="http://peplamb.com/linkable-title-html-and-php-widget/" target="_blank">Linkable Title Html and Php Widget</a>

== Changelog ==

= 1.2.4 =
* Maintenance Release.
* Upgraded GPL Licence from 2 to 3.

= 1.2.3 =
* Added: Twitter follow and share button.
* Added: Google Plus 1 button.
* Tested compatibility with Wordpress Version 3.5.1.

= 1.2.2 =
* Added: Options Page (Settings -&gt; Linkable Title Html and Php Widget ).
* Added: disable update nag option via the options page.
* Tested compatibility with Wordpress Version 3.5.

= 1.2.1 =
* Fixed: Div Tag (&lt;div class=&quot;textwidget&quot;&lt;) around the content wasn't echoing when "Remove Text Content Div Tag" option was unchecked its fixed now.
* Tested compatibility with Wordpress Version 3.4.2.

= 1.2.0 =
* Maintenance Release.
* Added Support: To add Title Text Color as Katie, Slips, Jim, Gary and John Herfindal request.
* Added Support: To remove Text Content Div Tag if needed.

= 1.1.9 =
* Maintenance Release.

= 1.1.8 =
* Maintenance Release.

= 1.1.7 =
* Tested compatibility with Wordpress Version 3.3.1.

= 1.1.6 =
* Disabled facebook like button which was blocking Save button thank you for letting me know Riley :)

= 1.1.5 =
* Bug fix: Thanks to Ben (Benjamin) for reporting the error.

= 1.1.4 =
* Fixed wrong function name.

= 1.1.3 =
* Maintenance Release.

= 1.1.2 =
* Force widget title underlined feature on Jane's request.

= 1.1.1 =
* Maintenance Release.

= 1.1.0 =
* Maintenance Release.

= 1.0.9 =
* Enhanced features.
* Tested with Wordpress 3.1.2

= 1.0.8 =
* Maintenance Release.

= 1.0.7 =
* Maintenance Release.

= 1.0.6 =
* FIX: "Automatically add paragraphs" will work perfectly now. (Thanks for reporting Sebastien).

= 1.0.5 =
* Some fixes.

= 1.0.4 =
* NEW: Added "Open the link/url in a new window" Support.

= 1.0.3 =
* Some fixes.

= 1.0.2 =
* Some fixes.

= 1.0.1 =
* fixed the issue where it wasn't saving the widget data.

= 1.0 =
* First release.

