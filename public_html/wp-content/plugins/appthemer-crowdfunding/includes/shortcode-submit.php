...
?>
	<h3 class="atcf-submit-section campaign-information"><?php _e( 'Thông tin dự án', 'atcf' ); ?></h3>

	<p class="atcf-submit-title">
		<label for="title"><?php _e( 'Tên dự án', 'atcf' ); ?></label>
...
?>
	<p class="atcf-submit-campaign-goal">
		<label for="goal"><?php printf( __( 'Mục tiêu (%s)', 'atcf' ), edd_currency_filter( '' ) ); ?></label>
	...
	<p class="atcf-submit-campaign-length">
		<label for="length">
			...
			<?php if ( ! atcf_has_preapproval_gateway() ) : ?>
				<a href="#" class="atcf-toggle-neverending"><?php _e( 'Không giới hạn ngày', 'atcf' ); ?></a>
			<?php endif; ?>
		</label>
...
?>
	<h4><?php _e( 'Hình thức đóng góp', 'atcf' ); ?> <?php if ( $edd_options[ 'faq_page' ] ) : ?><small> &mdash; <a href="<?php echo esc_url( get_permalink( $edd_options[ 'faq_page' ] ) ); ?>"><?php echo apply_filters( 'atcf_submit_field_type_more_link', __( 'Tìm hiểu thêm', 'atcf' ) ); ?></a></small><?php endif; ?></h4>
...
?>
	<p class="atcf-submit-campaign-category">
		<label for="category"><?php _e( 'Danh mục', 'atcf' ); ?></label>			
		<?php 
...
?>
	<div class="atcf-submit-campaign-description">
		<label for="description"><?php _e( 'Mô tả', 'atcf' ); ?></label>
		<?php 
...
?>
	<div class="atcf-submit-campaign-updates">
		<label for="description"><?php _e( 'Cập nhật', 'atcf' ); ?></label>
		<?php 
...
?>
	<p class="atcf-submit-campaign-excerpt">
		<label for="excerpt"><?php _e( 'Mô tả ngắn gọn', 'atcf' ); ?></label>
...
?>
	<p class="atcf-submit-campaign-images">
		<label for="excerpt"><?php _e( 'Hình ảnh giới thiệu', 'atcf' ); ?></label>
...
?>
	<p class="atcf-submit-campaign-video">
		<label for="video"><?php _e( 'Chèn link video clip giới thiệu dự án', 'atcf' ); ?></label>
...
?>
	...
	<p class="atcf-submit-campaign-shipping">
		<label for="shipping"><input type="checkbox" id="shipping" name="shipping" value="1" <?php checked(1, $shipping); ?> /> <?php _e( 'Ghi nhận thông tin mạnh thường quân', 'atcf' ); ?></label>
	</p>
	...
	<div class="atcf-submit-campaign-rewards">
		...
		<div class="atcf-submit-campaign-reward">
			...
			<p class="atcf-submit-campaign-reward-price">
				<label for="rewards[<?php echo esc_attr( $key ); ?>][price]"><?php printf( __( 'Số tiền (%s)', 'atcf' ), edd_currency_filter( '' ) ); ?></label>
			...
			<p class="atcf-submit-campaign-reward-description">
				<label for="rewards[<?php echo esc_attr( $key ); ?>][description]"><?php _e( 'Quyền lợi', 'atcf' ); ?></label>
			...
			<p class="atcf-submit-campaign-reward-limit">
				<label for="rewards[<?php echo esc_attr( $key ); ?>][limit]"><?php _e( 'Giới hạn số lượng', 'atcf' ); ?></label>
		...
		<div class="atcf-submit-campaign-reward">
			...
			<p class="atcf-submit-campaign-reward-price">
				<label for="rewards[0][price]"><?php printf( __( 'Số tiền (%s)', 'atcf' ), edd_currency_filter( '' ) ); ?></label>
				<input class="name" type="text" name="rewards[0][price]" id="rewards[0][price]" placeholder="<?php echo edd_format_amount( 20 ); ?>">
			</p>

			<p class="atcf-submit-campaign-reward-description">
				<label for="rewards[0][description]"><?php _e( 'Quyền lợi', 'atcf' ); ?></label>
				<input class="description" type="text" name="rewards[0][description]" id="rewards[0][description]" rows="3" placeholder="<?php esc_attr_e( 'Miêu tả quyền lợi cho mạnh thường quân', 'atcf' ); ?>" />
			</p>

			<p class="atcf-submit-campaign-reward-limit">
				<label for="rewards[0][limit]"><?php _e( 'Giới hạn số lượng', 'atcf' ); ?></label>
		...
		<p class="atcf-submit-campaign-add-reward">
			<a href="#" class="atcf-submit-campaign-add-reward-button"><?php _e( '[sofa_button link="#" mode="alt"]+ <em>Thêm quyền lợi</em>[/sofa_button]', 'atcf' ); ?></a>
		</p>
	</div>
<?php
}
...
?>
	<h3 class="atcf-submit-section payment-information"><?php _e( 'Thông tin của bạn', 'atcf' ); ?></h3>

	<?php if ( ! $atts[ 'editing' ] ) : ?>
		<p class="atcf-submit-campaign-contact-email">
		<?php if ( ! is_user_logged_in() ) : ?>
			<label for="email"><?php _e( 'Email', 'atcf' ); ?></label>
			<input type="text" name="contact-email" id="contact-email" value="<?php echo $atts[ 'editing' ] ? $campaign->contact_email() : null; ?>" />
			<?php if ( ! $atts[ 'editing' ] ) : ?><span class="description"><?php _e( 'Mọi thông tin tài khoản và liên lạc đều thực hiện qua email nên bạn phải chắc chắn điền email thực.', 'atcf' ); ?></span><?php endif; ?>
		<?php else : ?>
...
?>
	<p class="atcf-submit-campaign-author">
		<label for="name"><?php _e( 'Tên cá nhân hoặc tổ chức', 'atcf' ); ?></label>
...
?>
	<p class="atcf-submit-campaign-location">
		<label for="location"><?php _e( 'Địa chỉ', 'atcf' ); ?></label>
...
function atcf_shortcode_submit_before_success() {
	...
	$message = apply_filters( 'atcf_shortcode_submit_success', __( 'Gửi dự án thành công! Chúng tôi sẽ tiến hành xét duyệt trong thời gian sớm nhất', 'atcf' ) );
?>
...
function atcf_shortcode_submit_process() {
	...
	if ( isset( $edd_options[ 'show_agree_to_terms' ] ) && ! $terms )
		$errors->add( 'terms', __( 'Xin vui lòng đồng ý với điều khoản', 'atcf' ) );

	/** Check Title */
	if ( $title && '' == $title )
		$errors->add( 'invalid-title', __( 'Xin vui lòng nhập tên dự án.', 'atcf' ) );

	/** Check Goal */
	$goal = edd_sanitize_amount( $goal );

	if ( $goal && ! is_numeric( $goal ) )
		$errors->add( 'invalid-goal', sprintf( __( 'Vui lòng nhập số tiền mục tiêu chính xác. Đơn vị tính là %s .', 'atcf' ), $edd_options[ 'currency' ] ) );
	...
	/** Check Content */
	if ( $content && '' == $content )
		$errors->add( 'invalid-content', __( 'Bạn chưa nhập nội dung dự án.', 'atcf' ) );
	...
	if ( $image && empty( $image ) && atcf_theme_supports( 'campaign-featured-image' ) )
		$errors->add( 'invalid-previews', __( 'Bạn chưa chọn ảnh đại diện cho dự án.', 'atcf' ) );

	/** Check Rewards */
	if ( $rewards && empty( $rewards ) )
		$errors->add( 'invalid-rewards', __( 'Vui lòng nhập ít nhất một quyền lợi cho mạnh thường quân.', 'atcf' ) );

	if ( email_exists( $c_email ) && ! isset ( $current_user ) )
		$errors->add( 'invalid-c-email', __( 'Email này đã có người đăng ký rồi.', 'atcf' ) );		
