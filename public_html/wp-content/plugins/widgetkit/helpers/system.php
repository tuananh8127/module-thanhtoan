...
class SystemWidgetkitHelper extends WidgetkitHelper {
	...
	*/
    public function _adminInit() {
		...
		if ($this->active) {
			...
			// add actions
			add_action('in_admin_footer' , array($this, '_inAdminFooter'));
    ...
    public function _adminHead() {
        ...
        if ($this->active) {
            ...
            if (function_exists('wp_editor')) {
				...
				add_action( 'admin_footer', array( '_WP_Editors', 'enqueue_scripts'), 1 );
            }
        }
	...
	*/
    ...
    /*
		Function: _inAdminFooter
			In admin footer action callback, add a dummy editor to initialize tinyMCE

		Returns:
			Void
	*/
	public function _inAdminFooter() {
...
class WP_Widget_Widgetkit_Twitter extends WP_Widget {
	...
	public function form($instance) {
		...
		if(!function_exists('curl_init')) {
			echo "Please enable the PHP curl extension to use the Twitter widget";
			return;
		}
		...
		<p><strong>Twitter OAuth Settings</strong></p>
		
		<p>
			<label for="<?php echo $this->get_field_id('consumer_key'); ?>"><?php _e('Consumer Key:', 'warp'); ?></label>
			<input type="text" name="<?php echo $this->get_field_name('consumer_key'); ?>" value="<?php echo esc_attr($consumer_key); ?>" class="widefat" id="<?php echo $this->get_field_id('consumer_key'); ?>" />
		</p>		
		<p>
			<label for="<?php echo $this->get_field_id('consumer_secret'); ?>"><?php _e('Consumer Secret:', 'warp'); ?></label>
			<input type="text" name="<?php echo $this->get_field_name('consumer_secret'); ?>" value="<?php echo esc_attr($consumer_secret); ?>" class="widefat" id="<?php echo $this->get_field_id('consumer_secret'); ?>" />
		</p>		
		<p>
			<label for="<?php echo $this->get_field_id('access_token'); ?>"><?php _e('OAuth Access Token:', 'warp'); ?></label>
			<input type="text" name="<?php echo $this->get_field_name('access_token'); ?>" value="<?php echo esc_attr($access_token); ?>" class="widefat" id="<?php echo $this->get_field_id('access_token'); ?>" />
		</p>		
		<p>
			<label for="<?php echo $this->get_field_id('access_token_secret'); ?>"><?php _e('OAuth Access Token Secret:', 'warp'); ?></label>
			<input type="text" name="<?php echo $this->get_field_name('access_token_secret'); ?>" value="<?php echo esc_attr($access_token_secret); ?>" class="widefat" id="<?php echo $this->get_field_id('access_token_secret'); ?>" />
		</p>		
<?php
	}
