...
class EDD_Ipaymu {
	...
	/**
	 * Yeah, Because we need json_docode() function to retrieve session ID from Ipaymu
	 * So if the function don't exists we should throw a warning
	 *
	 * @since 1.0
	 */
	public function initialization_warning() {
		echo '<div id="edd-ipaymu-warning" class="updated fade"><p><strong>' . sprintf( __( '%s PHP library not installed.', 'edd-ipaymu' ), 'JSON' ) . '</strong> ';
		echo sprintf( __('EDD Ipaymu Payment Gateway plugin will not function without <a href="%s">PHP JSON functions</a> enabled. Please update your version of WordPress for improved compatibility and/or enable native JSON support for PHP.'), 'http://php.net/manual/book.json.php' );
		echo '</p></div>';
	}
